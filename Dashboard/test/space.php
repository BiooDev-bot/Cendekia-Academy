...
if(isset($_POST["submit"])){
    if($id < 20){
        $_SESSION["space"][$id]= $_POST["answer"];
        if($id<19){
            $id++;
            header("Location: space.php?q=$id");
            exit;
        }else{
            $jawaban=[];
            ...
            for($j=0;$j<$total_questions;$j++){
                if($jawaban[$j]==$_SESSION["space"][$j]){
                    $benar++;
                };
            }
            $nilai=$benar*5;
            $sum = "SELECT total FROM datalog WHERE username='$nama'";
            $append="INSERT INTO angkasa (username, score) VALUES ('$nama', $nilai)";
            ...
            mysqli_query( $conn, "UPDATE datalog SET space = 'yes' WHERE username = '$nama'");
        ...
        <?php for ($i = 0; $i < $total_questions; $i++): ?>
            <?php if($id==$i) :?>
                <div class="" style="background-color: aqua;">
                    <a href="space.php?q=<?=$i?>"><?php echo $i+1; ?></a>
                </div>
            <?php else : ?>
                    <div class="">
                        <a href="space.php?q=<?=$i?>"><?php echo $i+1; ?></a>
            ...
            <form method="POST">
                <div>
                    <input type="radio" name="answer" value="A" required 
                        <?php 
                            if (isset($_SESSION["space"][$id]) && $_SESSION["space"][$id] == "A") { 
                                echo "checked"; 
                            } 
                        ?>
                    ...
                    <input type="radio" name="answer" value="B" required 
                        <?php 
                            if (isset($_SESSION["space"][$id]) && $_SESSION["space"][$id] == "B") { 
                                echo "checked"; 
                            } 
                        ?>
                    ...
                    <input type="radio" name="answer" value="C" required 
                        <?php 
                            if (isset($_SESSION["space"][$id]) && $_SESSION["space"][$id] == "C") { 
                                echo "checked"; 
                            } 
                        ?>
                    ...
                    <input type="radio" name="answer" value="D" required 
                        <?php 
                            if (isset($_SESSION["space"][$id]) && $_SESSION["space"][$id] == "D") { 
                                echo "checked"; 
                            } 
                        ?>
                ...
                <div class="mt-3 d-flex justify-content-between">
                    <?php if ($id > 0): ?>
                        <a href="space.php?q=<?=$id - 1; ?>" class="btn btn-secondary">Kembali</a>
                    <?php endif; ?>
                    <?php if ($id < 19): ?>
